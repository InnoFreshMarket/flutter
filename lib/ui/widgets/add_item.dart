...
class _SellProductDialogState extends State<SellProductDialog> {
  ...
  final TextEditingController _numberForWholesaleController =
      TextEditingController();
  final TextEditingController _costWholesaleController =
      TextEditingController();
  final TextEditingController _numberController = TextEditingController();
  final TextEditingController _descriptionController = TextEditingController();
  bool _isSubscribed = false;
  final TextEditingController _numberForMonthSubscriptionsController =
      TextEditingController();
  String _category = "Фрукты";
  String? _completedDate;
  String? _willExpiredDate;
  String? _image;
  String? fileName;
          ...
          child: Column(
            children: [
              ...
              TextFormField(
                ...
                onChanged: (newValue) => setState(() {
                  _costRetailController.text = newValue;
                }),
              ),
              TextFormField(
                ...
                decoration: const InputDecoration(
                    labelText: "Минимальное количество для опта"),
                validator: (value) {
                  if (value!.isNotEmpty) {
                ...
                onChanged: (newValue) => setState(() {
                  _numberForWholesaleController.text = newValue;
                }),
              ),
              TextFormField(
                ...
                validator: (value) {
                  if (_numberForWholesaleController.text.isNotEmpty &&
                      value!.isEmpty) {
                    return "Пожалуйста введите цену";
                  }
                  if (value!.isNotEmpty) {
                ...
                onChanged: (value) => setState(() {
                  _costWholesaleController.text = value;
                }),
              ),
                ...
                onPressed: () async {
                  ...
                  if (pickedDate != null) {
                    setState(() {
                      ...
                      _completedDate = DateFormat('yyyy-MM-dd').format(pickedDate);
                    });
                  }
                },
                child: Text(_completedDate == null
                    ? "Выберите дату"
                    : _completedDate.toString()),
              ),
              const SizedBox(height: 16),
              TextFormField(
                ...
                keyboardType: TextInputType.number,
                decoration:
                    const InputDecoration(labelText: "Количество в наличии"),
                validator: (value) {
                  if (value!.isEmpty) {
              ...
              ElevatedButton(
                onPressed: () async {
                  ...
                  if (pickedDate != null) {
                    setState(() {
                      _willExpiredDate = DateFormat('yyyy-MM-dd').format(pickedDate);
                    });
                  }
                },
                child: Text(_willExpiredDate == null
                    ? "Выберите когда товар станет непригодным"
                    : _willExpiredDate.toString()),
              ),
              const SizedBox(height: 16),
              Row(
                children: [
              ...
              if (_isSubscribed) ...[
                TextFormField(
                  ...
                  decoration: const InputDecoration(
                      labelText:
                          "Максимальное доступное количество для подписки"),
                  validator: (value) {
                    if (value!.isEmpty) {
                      return "Пожалуйста введите количество товара";
                    }
                  ...
                  onChanged: (newValue) => setState(() {
                    _numberForMonthSubscriptionsController.text = newValue;
                  }),
                ),
              ],
              const SizedBox(height: 16),
              ...
              ElevatedButton(
                onPressed: () async {
                  if (_formKey.currentState!.validate() &&
                      _completedDate != null &&
                      _willExpiredDate != null &&
                      _image != null) {
                    final name = _nameController.text;
                    final costRetail =
                        double.tryParse(_costRetailController.text)!;
                    final numberForWholesale =
                        double.tryParse(_numberForWholesaleController.text);
                    final costWholesale =
                        double.tryParse(_costWholesaleController.text);
                    final number = double.tryParse(_numberController.text)!;
                    final description = _descriptionController.text;
                    final numberForMonthSubscriptions = double.tryParse(
                        _numberForMonthSubscriptionsController.text);
                    final item = Item(
                      name: name,
                      ...
                      number: number,
                      farmer: 1, // will be changed on backend
                    ...
                    var response = await ServerApi()
                        .post("/users/items/", token, item.toJson());
